...
export const clipboardActions: Pick<AppActions, 'copy' | 'paste'> = {
    copy() {
        if (!App.utils.isMainWorkspaceActive()) return;
        ...
        if (sel.active && sel.mask && sel.layerId === l.id) {
            ...
            const bbox = Lib.canvas.getContentBoundingBox(maskCanvas, 10);
            if (!bbox) return;

            const { canvas: maskedCanvas, ctx: mCtx } = Lib.canvas.create(l.canvas.width, l.canvas.height);
            ...
            const { canvas: croppedCanvas, ctx: cCtx } = Lib.canvas.create(bbox.w, bbox.h);
            cCtx.drawImage(maskedCanvas, bbox.x, bbox.y, bbox.w, bbox.h, 0, 0, bbox.w, bbox.h);
            ...
            clipData = {
                ...
                width: bbox.w,
                height: bbox.h,
                displayW: bbox.w * scaleX,
                displayH: bbox.h * scaleY,
                offsetX: bbox.x * scaleX + l.x,
                offsetY: bbox.y * scaleY + l.y,
                name: `${l.name} (Cropped Copy)`
            };
        } else {
            clipData = {
                canvas: Lib.canvas.clone(l.canvas),
