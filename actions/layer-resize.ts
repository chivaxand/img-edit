...
export const layerResizeActions: Pick<AppActions, 'openResizeDialog'> = {
    openResizeDialog() {
        ...
        const root = App.popup!.getById('rs-root')!;
        ...
        const constrainCheck = UI.createInput('checkbox', { checked: state.constrain }, (t: HTMLInputElement) => {
            ...
            if (state.constrain) updateInputs('w');
        });
        ...
        const lobesRow = UI.createSliderRow({
            ...
            onInput: (v: string) => state.lobes = parseInt(v)
        });
        ...
        root.appendChild(UI.createCheckbox({
            ...
            onChange: (v: boolean) => state.resizeCanvas = v
        }));

        root.appendChild(UI.createCheckbox({
            label: 'Resize Other Layers & Positions',
            value: state.resizeOthers,
            onChange: (v: boolean) => state.resizeOthers = v
        }));

        App.popup!.onClick('btn-cancel', () => App.popup!.close());
        App.popup!.onClick('btn-apply', () => {
            if (state.w < 1 || state.h < 1) return alert('Invalid dimensions');

            App.actions.saveState();

            // Calculate Scale Factors
            const sx = state.w / l.width;
            const sy = state.h / l.height;
